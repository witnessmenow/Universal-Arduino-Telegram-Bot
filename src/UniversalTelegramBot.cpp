  ...
  if (client->connected()) {
    ...
    if (getNextByteCallback == nullptr) {
        while (moreDataAvailableCallback()) {
            ...
            if (_debug) Serial.println(F("Sending photo from buffer"));
            }
    } else {
        if (_debug) Serial.println(F("Sending photo by binary"));
        byte buffer[512];
        int count = 0;
        char ch;
        while (moreDataAvailableCallback()) {
    ...
    if (_debug) Serial.print("End request: " + end_request);
    int ch_count = 0;
    now = millis();
...
bool UniversalTelegramBot::sendSimpleMessage(String chat_id, String text,
  ...
  bool sent = false;
  if (_debug) Serial.println(F("sendSimpleMessage: SEND Simple Message"));
  long sttime = millis();
...
bool UniversalTelegramBot::sendMessageWithReplyKeyboard(
  ...
  DynamicJsonDocument keyboardBuffer(1024); // assuming keyboard buffer will alwas be limited to 1024 bytes
...
String UniversalTelegramBot::sendPostPhoto(JsonObject payload) {
  ...
  bool sent = false;
  String response = "";
  if (_debug) Serial.println(F("sendPostPhoto: SEND Post Photo"));
  long sttime = millis();
...
String UniversalTelegramBot::sendPhotoByBinary(
  ...
  if (_debug) Serial.println(F("sendPhotoByBinary: SEND Photo"));
